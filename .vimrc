...
" Vim UI {
    ...
    if has('statusline')
        ...
        if !exists('g:override_zovim_bundles')
...
" Key (re)Mappings {
    ...
    let s:zovim_edit_config_mapping = '<leader>ev'
    let s:zovim_apply_config_mapping = '<leader>sv'
    ...
    "   let g:zovim_no_fastTabs = 1
    if !exists('g:zovim_no_fastTabs')
        map <S-H> gT
        map <S-L> gt
    endif

    " Stupid shift key fixes
    if !exists('g:zovim_no_keyfixes')
    ...
    "   let g:zovim_clear_search_highlight = 1
    if exists('g:zovim_clear_search_highlight')
...
" Functions {
    ...
    function! InitializeDirectories()
        ...
        "   let g:zovim_consolidated_directory = <full path to desired directory>
        "   eg: let g:zovim_consolidated_directory = $HOME . '/.vim/'
        if exists('g:zovim_consolidated_directory')
            let common_dir = g:zovim_consolidated_directory . prefix
        else
    ...
    execute "noremap " . s:zovim_edit_config_mapping " :call <SID>EditzovimConfig()<CR>"
    execute "noremap " . s:zovim_apply_config_mapping . " :source ~/.vimrc<CR>"
" }
